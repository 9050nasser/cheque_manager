...
frappe.ui.form.on("cheque", {
    onload: function (frm) {
        ...
        frm.set_query("cheque_treasury", function () {
            return {
                "filters": [
                    ["Account", "custom_cheque_type", "=", "Receive"]
                ]
            }
        });
        frm.set_query("cheque_treasury_from", function () {
            return {
                "filters": [
                    ["Account", "custom_cheque_type", "=", "Pay"]
                ]
            }
        });
    ...
    bank(frm) {
        frm.set_query("bank_account", function () {
            return {
                "filters": [
                    ["Bank Account", "bank", "=", frm.doc.bank]
                ]
            }
        });
    },
    party(frm) {
        frappe.call({
            ...
            args: {
                ...
                'fieldname': `${frm.doc.party_type.toLowerCase()}_name`

            },
            callback: function (r) {
                if (!r.exc) {
                    console.log(r.message)
                    frm.set_value("party_name", r.message[`${frm.doc.party_type.toLowerCase()}_name`]);
                }
            }
        });
    },
